...
public class SQL {
	public static final String MEMBER_INSERT=String.format("INSERT INTO %s(%s,%s,%s,%s,%s,%s,%s,%s)VALUES(?,?,?,?,?,?,?,now())", DB.TABLE_MEMBER,DB.MEMBER_ID, DB.MEMBER_NAME, DB.MEMBER_PASS, DB.MEMBER_SSN,DB.MEMBER_PHONE, DB.MEMBER_EMAIL, DB.MEMBER_PROFILE, DB.REGDATE);
	...
	public static final String BOARD_INSERT=String.format("INSERT INTO %s(%s,%s,%s,%s,%s,%s) VALUES (?,?,?,?,0,DATETIME)",DB.BOARD_ARTICLE_SEQ, DB.TABLE_BOARD,DB.BOARD_ARTICLE_SEQ,DB.MEMBER_ID, DB.TITLE, DB.BOARD_CONTENT, DB.BOARD_HITCOUNT,DB.REGDATE);
	...
	/*public static final String STUDENT_LIST= " select t.* "
				+ " from (select @rnum:=@rnum+1 NO, s.* "
				+ " from student s, (select @rnum :=0))t " 
				+ " where t.rnum between ? and ? ";
*/	
	public static final String STUDENT_LIST= " select c.* "
			+ " from (select @rnum := @rnum +1 as NO, t.* "
			+ " from student t, (select @rnum := 0) b ) c " 
			+ " where c.NO between ? and ? ";
...
/*	public static final String STUDENT_SEARCH=" select t2.* " +
	...
	public static final String STUDENT_SEARCH=" select t2.* " +
												" from (select @rnum := @rnum +1 as NO, s.* " +
												" from student s, (select @rnum := 0) b " +
												" where name like '%' || ? || '%' "  +
												" order by no desc) t2 "+
												" where t2.no between 1 and 5 ";
